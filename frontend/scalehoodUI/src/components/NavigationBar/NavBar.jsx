...
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { AppBar, Toolbar, InputBase, Typography, Box, Avatar } from '@mui/material';
import { styled, alpha } from '@mui/material/styles';
import SearchIcon from '@mui/icons-material/Search';
import logo from './hood.png'; // Update the path to your logo image
...
const NavBar = () => {
  ...
  return (
    <AppBar position="static" sx={{ background: '#FFFFFF', padding: '10px 0' }}>
      <Toolbar sx={{ maxWidth: '1000px', width: '100%', mx: 'auto' }}>
        
        {/* Logo and App Name */}
        <Box display="flex" alignItems="center" sx={{ mr: 2 }}>
          <Avatar src={logo} alt="ScaleHood logo" sx={{ height: 40, width: 40, mr: 1 }} />
          <Typography variant="h5" sx={{ color: '#6bcab5', fontWeight: 'bold' }}>ScaleHood</Typography>
        </Box>
        ...
        {/* User Greeting */}
        <Box sx={{ ml: 'auto' }}>
          <Typography variant="h6" sx={{ color: '#6bcab5', fontWeight: 700 }}>Hi Mohit!</Typography>
        </Box>
      </Toolbar>
    </AppBar>
  );
...
const Search = styled('div')(({ theme }) => ({
  position: 'relative',
  borderRadius: theme.shape.borderRadius,
  backgroundColor: alpha(theme.palette.grey[300], 0.7), // Light gray background
  '&:hover': {
    backgroundColor: alpha(theme.palette.grey[300], 0.9), // Darker on hover
  },
  marginRight: theme.spacing(2),
  width: '100%',
  [theme.breakpoints.up('sm')]: {
